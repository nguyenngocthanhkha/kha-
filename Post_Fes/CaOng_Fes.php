<?php include('../Includes/header.php'); ?>
<!DOCTYPE html>
<html lang="vi">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Lễ hội Rước cá Ông</title>
      <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
    <link rel="stylesheet" href="../CSS/stylePost.css">
    <link rel="stylesheet" href="../CSS/styleHeader.css">
    <link rel="stylesheet" href="../CSS/styleFooter.css">
    <link
        href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;700&family=Merriweather:wght@400;700&display=swap"
        rel="stylesheet">
</head>

<body>
    <article class="style2">

        <h1>🎉 Lễ hội Rước cá Ông – “Big Event” của dân biển Nhơn Hải 🎏🌊</h1>
        <p>🗓 Từ ngày 11/3 đến 14/3 dương lịch (tức 11 - 14/2 âm lịch), xã biển Nhơn Hải (TP. Quy Nhơn, Bình Định) rộn
            ràng tổ chức lễ hội cầu ngư – một “sự kiện” văn hóa truyền thống cực chất, thu hút cả dân bản địa lẫn khách
            du lịch đổ về xem đông như trẩy hội.</p>
        <h2>🐋 Cá Ông – Vị thần "hộ thân" của dân đi biển</h2>
        <p>Ngư dân ở đây tin tưởng cá Ông (tức cá voi) là vị thần đại dương siêu linh, luôn "gánh team", bảo vệ họ khỏi
            sóng to gió lớn khi ra khơi. Truyền thuyết kể rằng cá Ông là hóa thân từ áo cà sa của Phật Bà Quan Âm, quăng
            xuống biển để cứu người bị nạn. Nên dân biển tôn kính hết nấc!</p>
        <figure>
            <img src="../Picture/CaOng_0.png"
                alt="Rước thần">
            <figcaption>Rước thần Nam Hải từ bãi biển về lăng</figcaption>
        </figure>
        <h2>🧧 Khi cá Ông "lụy" – cả làng tổ chức "quốc tang mini"</h2>
        <p>Nếu có cá Ông dạt vào bờ, làng nào nhận “cứu trợ” sẽ tổ chức lễ tang cực trang trọng: tắm rượu, liệm vải đỏ,
            chôn cất tử tế, sau 3 năm còn có lễ cải táng hoành tráng đưa vào lăng. Nghiêm túc và thành kính không thua
            gì đám tang con người.:</p>
        <figure>
            <img src="../Picture/CaOng_1.png"
                alt="Dải lụa đỏ – cầu để thần vào chánh điện">
            <figcaption>Dải lụa đỏ – cầu để thần vào chánh điện</figcaption>
        </figure>
        <h2>🎭 Lễ nghi + phần hội = combo văn hóa – giải trí siêu chill</h2>
        <p>Ngày 11 âm lịch, lễ chính tế Nam Hải Thần Ngư bắt đầu. Dân chài cầu mong mùa cá đầy ghe, trời yên biển lặng
            và cả bình an cho xóm làng. Ban đêm còn có phần múa gươm, múa bá trạo, đọc văn tế, cực “deep”.</p>
        <figure>
            <img src="../Picture/CaOng_2.png"
                alt="Đội gươm theo rước thần">
            <figcaption>Đội gươm theo rước thần</figcaption>
        </figure>
        <p>Ngày tiếp theo thì đúng nghĩa "hội là chính": hát tuồng, kéo co, đua thuyền, lắc thúng, nhảy bao bố... Người
            chơi thì vui, người xem thì quên cả về!</p>
        <h2>🎊 Giao lưu văn hóa – chill từ đêm đến sáng</h2>
        <p>Tầm 1h30 sáng – ai "trâu" thì ráng ở lại xem đội gươm múa hầu thần nha, vibe cực cháy. Đặc biệt, từ năm 2013
            còn có đội bá trạo mới toanh, thêm phần hấp dẫn cho lễ hội.</p>
        <figure>
            <img src="../Picture/CaOng_3.png"
                alt="Đội gươm múa hầu thần vào lúc 1 giờ 30 phút sáng trong chánh lễ tế thần Nam Hải.">
            <figcaption> Đội gươm múa hầu thần vào lúc 1 giờ 30 phút sáng trong chánh lễ tế thần Nam Hải</figcaption>
            <h2>💬 Không chỉ lễ, mà còn là dịp “bonding” cộng đồng</h2>
            <p>Lễ hội là thời điểm ngư dân gặp nhau, giao lưu, trao đổi kinh nghiệm đi biển và… xả stress sau những ngày
                lao động cực nhọc. Ai nói dân biển không chill?</p>
        </figure>
        <p>💡 Tóm lại: Rước cá Ông không chỉ là một lễ hội tâm linh, mà còn là “đặc sản tinh thần” mang đậm bản sắc dân
            gian miền biển – truyền thống có, vui có, chất Gen Z thì không thiếu!</p>
    </article>

</body>

</html>
<?php include('../Includes/footer.php'); ?>